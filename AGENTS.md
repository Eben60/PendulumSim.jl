# AI Agent Guidelines for PendulumSim
...
**Important**: Agents can view terminal output even when detached from the REPL. The terminal context provides snapshots of recent command output from both zsh and Julia REPL terminals, including stderr/stdout from background processes and exit codes. This allows monitoring of running processes without direct interaction.
...
**Note**: Even when detached from the REPL, you can monitor terminal output. Check terminal context regularly to see command results, errors, and background process output without needing to interact directly with the REPL.
...
The main module is `PendulumSim`, defined in `src/PendulumSim.jl`.
